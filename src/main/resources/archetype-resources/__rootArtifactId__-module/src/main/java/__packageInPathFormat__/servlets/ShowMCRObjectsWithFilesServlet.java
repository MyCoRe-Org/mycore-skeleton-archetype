...
public class ShowMCRObjectsWithFilesServlet extends HttpServlet {
    ...
    private void outputData(PrintWriter pw) {
        ...
        try {
            ...
            final QueryResponse queryResponse = MCRSolrCoreManager.getMainSolrClient().query(query);
            ...
            if (queryResponse.getResults().getNumFound() > 0) {
                ...
                for (SolrDocument doc : queryResponse.getResults()) {
                    ...
                    pw.print("<td>"
                        + ("true".equals(doc.getFirstValue("${rootArtifactId}.hasFiles").toString()) ? "ja" : "nein")
                        + "</td>");
                    pw.println("      </tr>");
                }
                pw.println("    </table>");
